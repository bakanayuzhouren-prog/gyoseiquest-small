            ...
            if (questionCount === 0) {
                for (let j = i; j < linesArr.length; j++) {
                    if (linesArr[j].includes('"explain":') || linesArr[j].includes("'explain':")) {
                        linesArr[j] = `        "explain": "${rich_explain_with_two_images}",`;
                        updated = true;
                        break;
                    }
...
if (updated) {
    ...
    console.log('Flexible image tag inserted into Index 0!');
} else {
