...
const QUESTIONS_TEMP = 'src/questions_merged.mjs';
...
import { SUBJECTS } from './src/questions_merged.mjs';

const LEARN_JS = 'src/learn.js';

console.log('--- Phase 2: Updating learn.js (High Precision Fixed) ---');
...
for (const subject in LEARN_CONTENT) {
    ...
    updatedLearnContent[subject] = LEARN_CONTENT[subject].map((text, index) => {
        // Find corresponding SUBJECTS key (might have spaces or slight differences)
        ...
        for (const subKey in SUBJECTS[matchKey]) {
            ...
            for (const q of questions) {
                ...
                // Match by starting with 15 chars (very lenient for learn content segments)
