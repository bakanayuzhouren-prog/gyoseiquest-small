            ...
            if (questionCount === 0) {
                for (let j = i; j < linesArr.length; j++) {
                    if (linesArr[j].includes('"explain":') || linesArr[j].includes("'explain':")) {
                        linesArr[j] = `        "explain": "${rich_explain_with_image}",`;
                        updated = true;
                        break;
                    }
...
if (updated) {
    ...
    console.log('Image tag inserted into Index 0!');
} else {
