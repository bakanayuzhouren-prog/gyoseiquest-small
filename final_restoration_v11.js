                ...
                questions.forEach((q, i) => {
                    ...
                    if (match) {
                        ...
                        if (tag.toLowerCase().startsWith('[[image')) {
                            const innerMatch = match[1].match(/image\s*:\s*(.*)/i);
                            const inner = innerMatch ? innerMatch[1].trim() : 'MISSING';
                            tag = `[[image:${inner}]]`;
                        }
                        if (!arr[i]) arr[i] = tag;
                    }
...
crawlForImages(subjectsBak);
crawlForImages(subjectsBackup);
...
for (const subNameRaw in oldLearnOrig) {
    ...
    if (sourceArr) {
        ...
        console.log(`Subject "${subName}": Skipping (No image data found)`);
    }
