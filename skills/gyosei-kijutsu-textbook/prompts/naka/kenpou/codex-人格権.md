# Codex用 — 人格権

てらしぃが**このファイルだけ**を Codex に渡す。1論点・1枚。Cursor は画像を作らない。  
Google／Twitterの物差し比較は別ファイル。

- 保存: `assets/images/deepdive/naka/kenpou/bougai-ladder.png`
- キー: `naka/kenpou/bougai-ladder`
- 正本: `N・C/憲法/2026-人格権妨害排除.md`

## 法律（守る）

- 占有訴権に妨害排除（198条）
- 物権・所有権は解釈
- 人格権にも認められる。削除＝妨害排除の一種
- **憲法13条根拠と書かない**（私法上の人格権）
- 北方ジャーナル（出版の事前差止め）と棚が違う

## 禁止

- GO と YES 混在／だれが／問が聞くこと／（聞かない）

## 役割

- 権利者（消してほしい）

## GPT Image プロンプト（このまま生成）

```text
Create a NEW Japanese legal-study infographic from scratch for Gyosei Quest / あぷし. ONE TOPIC ONLY: 妨害排除の根拠が占有→物権→人格権へ広がる. Do not compare Google vs Twitter standards as the main story.

Match LAYOUT of「主宰者の許可」: navy title, left green「論点」, right orange「ひっかけ」, ONE center metaphor, bottom 判断軸 / ひっかけ / 暗記, navy answer bar. 16:9 warm off-white. Large Japanese. No overlap.

Guide: ちゃちゃロット SMALL bottom-right owl slot, 指し棒 pointing at 暗記. Match chachalot.png. Not a scene character. No nameplate. Not bear/owl/cat.
(pale-sky-blue HAT not ears, cream face, equal circle eyes, four cheek marks, no glasses).

STRICT:
- Left header「論点」. Never「問が聞くこと」.
- Right header「ひっかけ」. Never「（聞かない）」.
- 論点: Q&A only. NO GO/STOP. Do not mix GO and YES.
- Never write「だれが」.
- Do NOT write 憲法13条 as the legal ground.

Title:「妨害排除 — 占有 → 物権 → 人格権」
Chip:「憲法13条根拠と書くな」

Center: three widening rings: 占有訴権（198条）→ 物権的請求（解釈）→ 人格権.
Labels MUST be:
「権利者（消してほしい）」

Left 論点:
1. 占有訴権に妨害排除？ → YES（198条）
2. 所有権は？ → 解釈で認められる
3. 人格権は？ → 認められる（削除＝妨害排除の一種）

Right ひっかけ:
- 憲法13条で保障された人格権
- 北方ジャーナルの事前差止めと同じ
- 占有回収（200条）と混ぜる

Bottom:
- 判断軸:「根拠が広がる。ネット削除は私法上の人格権」
- ひっかけ:「13条／検閲／占有回収」
- 暗記:「占有→物権→人格権。削除は妨害排除の一種」
Answer EXACT:
「削除請求は私法上の人格権に基づく妨害排除の一種である。」
```
