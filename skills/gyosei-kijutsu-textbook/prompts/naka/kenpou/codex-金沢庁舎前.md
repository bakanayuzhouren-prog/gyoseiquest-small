...
- 保存: `assets/images/deepdive/naka/kenpou/kanazawa.png`
- キー: `naka/kenpou/kanazawa`

## 法律（守る）

- 最三小判令5.2.21。庁舎前広場での政治的示威の禁止適用は**21条1項に違反しない**
- 広場は庁舎と**一体**。パブリックフォーラムに変容しない
- 他の公の施設は使えるので制限は限定的
- 泉佐野（会館は原則貸す）と棚が違う
...
```text
Create a NEW Japanese legal-study infographic from scratch for Gyosei Quest / あぷし. ONE TOPIC ONLY: 金沢市市庁舎前広場事件. Do not decide 泉佐野 as the main rule.
...
Center: city hall and front plaza fused as 一体. Protest sign with stamp「中立の外観を害する」. Other parks usable in the distance.
Labels MUST be:
「示威したい人（庁舎前で訴えたい）」

Left 論点:
1. 21条違反？ → この適用は違反しない（合憲）
2. 広場はパブリックフォーラムに変容？ → NO（庁舎と一体）
3. 他の施設は？ → 使えるので制限は限定的
...
- 判断軸:「庁舎の一部か、市民会館か」
- ひっかけ:「泉佐野と同じ」
- 暗記:「庁舎前の示威禁止は合憲。会館とは別」
Answer EXACT:
「庁舎前広場での政治的示威の禁止は、庁舎と一体であり21条に違反しない。」
```
