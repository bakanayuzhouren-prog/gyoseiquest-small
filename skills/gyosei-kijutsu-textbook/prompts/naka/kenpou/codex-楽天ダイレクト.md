...
- 保存: `assets/images/deepdive/naka/kenpou/rakuten.png`
- キー: `naka/kenpou/rakuten`

## 法律（守る）

- 最一小判令3.3.18。要指導医薬品の**対面**義務
- **合憲**。規制目的**二分論は述べない**
- **態様規制**。市場1％未満・期間限定。職に就けない話ではない
- 電話・メールは対面に劣る（理解の確認）
- 旧警備業法＝狭義・違憲。この事件＝態様・合憲
...
```text
Create a NEW Japanese legal-study infographic from scratch. ONE TOPIC ONLY: 楽天ダイレクト事件. Do not hold 旧警備業法 unconstitutional on this image.
...
- Never write「だれが」.
- Do NOT use 規制目的二分論. Conclusion is 合憲.

Title:「楽天ダイレクト — 態様規制は合憲」
Chip:「二分論なし。市場1％未満」

Center: pharmacy counter labeled 対面. Tiny online cart tagged「1％未満」. Phone/mail stamped 対面に劣る.
Labels MUST be:
「販売業者（ネットで売りたい）」
...
Bottom:
- 判断軸:「制限の性質。態様で程度も大きくない」
- ひっかけ:「二分論／警備と同じ違憲」
- 暗記:「要指導薬の対面は態様。合憲」
Answer EXACT:
「要指導医薬品の対面販売義務は職業活動の態様規制であり合憲。」
```
