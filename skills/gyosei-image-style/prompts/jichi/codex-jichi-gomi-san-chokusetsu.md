...
語呂（てらしぃ指定）: **ゴミさん** ＝ 50（ゴ）と 3（サン）。やさしい請求は50分の1、重い請求は3分の1。
...
住民監査請求（242条）は住民1人で可。ゴミさんの連署表の外。
...
| 中央メタファー | 4行表。連署の右にルビ（ゴ／サン） |
| 判断軸 | やさしい請求か、重い請求か。請求先は誰か |
| ひっかけ | 全部1/50。条例は議会へ。住民監査も1/50。過半数もスライド |
| 暗記 | ゴミさん。条例・事務監査は50分の1。解散・解職は3分の1 |
| 役割 | 語呂の体現（連署の数字を覚える）／案内役 |

## 配役（てらしぃ個別指定）

語呂の場面役は許可名簿にいない **ゴミさん**（ごみ袋とサンバを一身に持つ）。参照PNGはない。いい役。ちゃちゃロットは案内役のまま1体。カチャドクロ系は置かない。

## GPT Image プロンプト

```text
参照必須: ちゃちゃロットの全身指し棒正本 skills/gyosei-image-style/assets/approved-chachalot-pointer.png を照合する。説明行には（〇条）を出す。場面役のゴミさんは参照PNGなし。許可名簿のマスコットに似せるな。
Create a NEW Japanese legal-study infographic from scratch.
ONE job: 地方自治の直接請求。やさしい請求は50分の1、重い請求は3分の1。語呂はゴミさん（ゴ＝50、サン＝3）.
...
議会の解散（76条） | 3分の1 | サン | 選管
議員・長の解職（80条・81条） | 3分の1 | サン | 選管
The 語呂 cell is ruby-like: small bold katakana sitting immediately to the RIGHT of the fraction, like furigana beside the digits. 50分の1＋ゴ, 3分の1＋サン.
Caption under the table, large and playful:「ダジャレで覚えようぜ直接請求」then smaller「ゴミさん＝50分の1と3分の1。税条例は除外。住民監査（242条）は1人で可・連署なし。解散の連署だけ人口スライド。投票の過半数は緩めない」

Scene: below the table, dedicated band for ゴミさん. Adult Japanese person as a sanitation-worker-plus-samba gag (one garbage bag, one samba sash), energetic, not sexualized, not a child, not a mascot animal. Label「語呂の体現（連署の数字を覚える）」. Must not cover the table, ruby, caption, bottom cards, or answer bar. No owl, bear, cat, raccoon. No ちゃちゃロット as ゴミさん. Do not draw カチャドクロ.

Bottom layout: navy answer bar is the very bottom edge. Directly ABOVE the answer bar, at the right end, reserve a dedicated warm-off-white guide safe zone. Keep the three bottom cards within the left approximately 80% of their row and reserve the right approximately 20% for the guide. The guide must not overlap the table, ゴミさん, the bottom cards, body text, or the answer bar.
