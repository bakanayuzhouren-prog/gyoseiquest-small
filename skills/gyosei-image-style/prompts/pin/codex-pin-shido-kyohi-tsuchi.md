# ピン・行政指導に応じなかった旨の通知（処分性なし）

- 保存先: assets/images/deepdive/learn/pin/shido-kyohi-tsuchi.png
...
配置（生成後・Cursor）: ピン `administrative_guidance_refusal`（表から差し替え）。姉妹は `hospital_abolition`。
