...
| 役 | やること |
|---|---|
| **Cursor** | Codex 用 `codex-*.md` を作る／**上書きする**。`保存先:` を書く。**画像は描かない。** アプリ載せは生成後 |
| **Codex** | 未生成 PNG だけ、**古いプロンプトから順に** GPT Image で作る。**プロンプトは直さない。** |

Codex はプロンプトを新規作成しない・上書きしない。実装・sync・X予約もしない。
GPT Image 用本文（` ```text `）に口語を禁止語として載せない（載せると生成できない）。正しい法律用語だけ。
...
3. 保存先の PNG が既にある枚は、**描かない**。`上書き対象:` や `overwrite: true` が残っていても上書きしない。てらしぃが今の指示で、その画像の修正をファイル名で明示したときだけ、その1枚を上書きする。未生成の pending だけ描く。
4. **`pending` がある**なら、一覧の **上から（mtime 古い順）** 1ファイルずつ開く。
5. **生成する前に** `PRE-GENERATE-CHECK.md` を通す。おかしい点があれば **その枚は生成しない**（てらしぃへ報告。修正前ファイルは直さない。次の pending へ進んでよい）。
6. チェック全OKのときだけ、そのファイルの GPT Image プロンプトで **1枚**生成する。型が崩れたら次に進まない。
