...
const input = 'data/moshi/goukaku-kakumei-round1-topics.json';
...
for (const t of confirmed) {
  ...
  (bySubject[learnSubject] ??= []).push({
    text: `【合格革命・問${t.questionNumber}】${t.memory}`,
    deepdive: `■ 結論\n\n${t.rule}\n\n■ なぜそうなる\n\n${t.deepDive}\n\n■ ひっかけ\n\n[[red:${t.trap}]]\n\n■ 暗記\n\n${t.memory}`,
    fExplain: t.aim,
    statuteRef: (t.references || []).join('、'),
    source: `合格革命模試 第1回 問${t.questionNumber}`,
  });

  bonus[quizSubject] ??= {};
  (bonus[quizSubject][quizField] ??= []).push({
    text: `【ボーナス合格革命・問${t.questionNumber}系】${t.practiceQuestion.prompt}`,
...
fs.writeFileSync('data/knowledge/creator/prep-school/goukaku-kakumei-round1-topics.md', lines.join('\n') + '\n');
const banner = '// Generated by scripts/buildMoshiTopicLearning.mjs. Edit the JSON source.\n';
fs.writeFileSync(
  'src/goukaku_moshi_learn_content.js',
  banner + `export const GOUKAKU_MOSHI_LEARN_BY_SUBJECT = ${JSON.stringify(bySubject, null, 2)};\n`,
);
fs.writeFileSync(
  'src/goukaku_moshi_bonus_questions.js',
  banner + `export const GOUKAKU_MOSHI_BONUS_QUESTIONS = ${JSON.stringify(bonus, null, 2)};\n`,
);
console.log(`Validated and generated ${confirmed.length} topics. Missing questions: ${missing.join(',')}`);
