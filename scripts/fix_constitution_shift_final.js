...
function fixFullConstitutionSection() {
    ...
    // Find the end: look for the start of the next major section "行政法"
    for (let i = kenStart + 1; i < lines.length; i++) {
        if (lines[i].includes('"行政法": [')) {
            kenEnd = i - 1;
            console.log(`Constitution end detected before "行政法" at line ${i}`);
            break;
        }
    }
    ...
    console.log(`Found ${explains.length} explain lines in Constitution section.`);
