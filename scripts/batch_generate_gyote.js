...
const BACKUP_FILE = path.join(__dirname, '../src/questions.js.backup_ai_batch');
...
function loadData() {
    ...
    // Extract STATUTES for context (Not used in lightweight mode, but kept for future reference if needed)
    const statutesMatch = content.match(/export const STATUTES = ({[\s\S]+?});/);
    const statutesJson = statutesMatch ? statutesMatch[1] : "{}";

    return {
        content,
        subjects: JSON.parse(subjectsJson),
        statutes: JSON.parse(statutesJson)
    };
}
...
**あなたの学習済みの「行政手続法」の条文知識をフル活用し**、以下の「5つの問題」について、それぞれの解説を一括で作成してください。
...
async function main() {
    ...
    const { content, subjects, statutes } = loadData();

    // Statutes Text is NO LONGER NEEDED for this lightweight mode.
    // const gyoteStatutes = statutes["gyote"] || [];
    // const statutesText = gyoteStatutes.map(s => `【${s.title}】\n${s.content}`).join("\n\n");

    // Target: 行政法 > 行政手続法
    const targetQuestions = subjects["行政法"] ? subjects["行政法"]["行政手続法"] : null;
    if (!targetQuestions) {
        console.error("No questions found for 行政法 > 行政手続法");
        return;
    }
